...
const Total = () => {
    ...
    const tax = useSelector(state => state.total.tax)
    const disCount = useSelector(state => state.total.discount)
    const shipping = useSelector(state => state.total.shipping)
    const dispatch = useDispatch()
    const total = useSelector (state => state.total)
    console.log('total is equal to = ', total)
    const lastTotal = (sumOfAmounts + (sumOfAmounts * (tax/100))) + (shipping - disCount)
        ...
        <div className='w-[377px] h-[full]'>
                      <div className='w-full h-[292px] flex flex-col'>
                        ...
                        <div className='w-[377px] h-[26px] mb-[24px] flex justify-between '>
                          ...
                          <div className='w-[251.33px] h-[26px] flex justify-end mr-[12px]'>{currency} {isNaN(sumOfAmounts) ? 0 : sumOfAmounts}</div>
                        </div>
